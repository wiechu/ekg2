...
/*
 * ncurses_display_indent_changed ()
 *
 * called when var display_mode is changed
 */
static void ncurses_display_indent_changed(const char *var) {
	window_t *w;
...
static int ncurses_theme_init() {
	...
	format_add("statusbar", " %c(%w%{time}%c)%w %c(%w%{?session %{?away %G}%{?avail %Y}%{?chat %W}%{?dnd %K}%{?xa %g}%{?gone %R}%{?invisible %C}%{?notavail %r}%{session}}%{?!session ---}%c) %{?window (%wwin%c/%w%{?typing %C}%{window}}%{?query %c:%W%{query}}%{?debug %c(%Cdebug}%c)%w%{?activity  %c(%wact%c/%W}%{activity}%{?activity %c)%w}%{?mail  %c(%wmail%c/%w}%{mail}%{?mail %c)}%{?more  %c(%G%{more}%c)}", 1);
...
/* redraw userlisty: */
	...
	variable_add(&ncurses_plugin, ("display_indent"), VAR_INT, 1, &config_display_indent, ncurses_display_indent_changed,
			variable_map(3, 0, 0, "timestamp-always", 1, 2, "indent-prompted", 2, 1, "indent-all"), NULL);
	...
	variable_add(&ncurses_plugin, ("mouse_scroll_lines"), VAR_INT, 1, &config_mouse_scroll_lines, NULL, NULL, NULL);
