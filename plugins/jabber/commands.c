...
COMMAND(jabber_command_connect);	/* in jabber.c */
...
static COMMAND(jabber_command_disconnect)
{
	...
	/* jeśli jest /reconnect, nie mieszamy z opisami */
...
/* w libtlenie jest <show>unavailable</show> + eskejpiete tlen_encode() */
	...
	if (session->connected) {
		...
		if (lt)
			jabber_write(session, "<message type='chat' to='%s'>"
		...
		if (descr) {
			...
			jabber_write(session, "<presence type='unavailable'><status>%s</status></presence>", tmp ? tmp : "");
			xfree(tmp);
		} else
			jabber_write(session, "<presence type='unavailable'/>");
	}

	if (!j->istlen) jabber_write(session, "</stream:stream>");
	else		jabber_write(session, "</s>");
...
/* writing: */
	...
	if (ismuc)
		...
		jabber_write(session, "<message %sto='%s' id='%d'>",
			chat ? "type='chat' " : "",
/*				j->istlen ? "type='normal' " : "",  */
			uid+5, time(NULL));

	if (subject) {
		jabber_write(session, "<subject>%s</subject>", subject);
		xfree(subject);
	}
	if (thread) {
		jabber_write(session, "<thread>%s</thread>", thread);
		xfree(thread);
	}
	...
	if (!secure /* || j->istlen */) {
		...
		jabber_write(session, "<body>%s</body>", tmp);
		xfree(tmp);
	}
	if (!j->istlen)
...
nomsg:
	if (htmlmsg) {
		jabber_write(session, "%s", htmlmsg);
		xfree(htmlmsg);
	}

	if (!j->istlen)
		jabber_write(session, "<x xmlns='jabber:x:event'>%s%s<displayed/><composing/></x>",
...
static COMMAND(jabber_command_auth) {
	...
	if (!xstrcmp(target, "*")) {
		...
		tabnick_add(__uid);	/* user jest OK, więc lepiej mieć go pod ręką */
	...
	do {
		...
		jabber_write(session, "<presence to='%s' type='%s' id='roster'/>", uid+5, action);
		xfree(uid);
...
static COMMAND(jabber_command_modify) {
	...
	if (addcom) {
		...
		for (i = 0; argv[i]; i++) {
			if (match_arg(argv[i], 'g', ("group"), 2) && argv[i + 1]) {
				...
				int x, off;	/* jeśli zaczyna się od '@', pomijamy pierwszy znak */
	...
	if (nickname)	jabber_write(session, "<item jid='%s' name='%s'%s>", uid+5, nickname, (u->groups ? "" : "/"));
	else		jabber_write(session, "<item jid='%s'%s>", uid+5, (u->groups ? "" : "/"));

	for (gl = u->groups; gl; gl = gl->next) {
		struct ekg_group *g = gl;
		char *gname = tlenjabber_escape(g->name);

		jabber_write(session, "<group>%s</group>", gname);
		xfree(gname);
	}
...
static COMMAND(jabber_command_del) {
	...
	if (del_all) {
		...
		for (ul = session->userlist; ul; ul = ul->next) {
			...
			jabber_write(session, "<item jid='%s' subscription='remove'/>", u->uid+5);
		}
	...
	jabber_write(session, "<iq type='set' id='roster'><query xmlns='jabber:iq:roster'><item jid='%s' subscription='remove'/></query></iq>", u->uid + 5);
...
#define strfix(s) (s ? s : "")
	...
	jabber_write(session, "<iq type='set'><vCard xmlns='vcard-temp'>"
...
static COMMAND(jabber_command_search) {
	...
	if (splitted) {
		...
		if (!xstrcmp(splitted[0], "jabber_x_data")) {
			use_x_data = 1; i = 2;
			jabber_write(session, "<x xmlns='jabber:x:data' type='submit'>");
		}

		for (; (splitted[i] && splitted[i+1]); i+=2) {
			char *value = jabber_escape(splitted[i+1]);
			if (use_x_data)
				jabber_write(session, "<field var='%s'><value>%s</value></field>", splitted[i], value);
			else	jabber_write(session, "<%s>%s</%s>", splitted[i], value, splitted[i]);
			xfree(value);
		}

		if (use_x_data) jabber_write(session, "</x>");
	}
	jabber_write(session, "</query></iq>");
	g_strfreev(splitted);
...
static COMMAND(jabber_command_privacy) {	/* jabber:iq:privacy in ekg2 (RFC 3921) by my watch */
	...
	if (!params[0] || !xstrcmp(params[0], "--lists")) {
		...
		jabber_write(session, "<iq type='get' id='%s'><query xmlns='jabber:iq:privacy'/></iq>", id);
		return 0;
	}
	...
	if (!xstrcmp(params[0], "--session") || !xstrcmp(params[0], "--default")) {
		...
		if (unset)	jabber_write(session, "<iq type='set' id='privacy%d'><query xmlns='jabber:iq:privacy'><%s/></query></iq>", j->id++, tag);
		else		jabber_write(session, "<iq type='set' id='privacy%d'><query xmlns='jabber:iq:privacy'><%s name='%s'/></query></iq>", j->id++, tag, val);
		xfree(val);
		return 0;
	}
	...
	if (!xstrcmp(params[0], "--get")) {		/* display list */
		...
		jabber_write(session, "<iq type='get' id='privacy%d'><query xmlns='jabber:iq:privacy'><list name='%s'/></query></iq>", j->id++, val ? val : "ekg2");
		xfree(val);
		return 0;
	}
...
privacy_delete_ok:
 ...
 /*				denylist->allow = 0; */
	...
	if (((needsync && !j->privacy) || !xstrcmp(params[0], "--unset"))) {
		...
		jabber_write(session, "<iq type='set' id='privacy%d'><query xmlns='jabber:iq:privacy'><list name='%s'/></query></iq>", j->id++, val ? val : "ekg2");
		xfree(val);
		return 0;
	}
	...
	if ((needsync || !xstrcmp(params[0], "--sync")) && j->privacy) {
		...
		/* XXX XXX */
			...
			jabber_write(session, "<item type='%s' value='%s' action='%s' order='%d'>",
				p->type,												/* type (jid/group/subscription)*/
				eval,													/* value */
				p->allow ? "allow" : "deny",										/* action */
				p->order);												/* order */

			if (p->items & PRIVACY_LIST_MESSAGE)		jabber_write(session, "<message/>");
			if (p->items & PRIVACY_LIST_IQ)			jabber_write(session, "<iq/>");
			if (p->items & PRIVACY_LIST_PRESENCE_IN)	jabber_write(session, "<presence-in/>");
			if (p->items & PRIVACY_LIST_PRESENCE_OUT)	jabber_write(session, "<presence-out/>");
...
/*							else											book->priv_data.conf->autojoin = 0; */
	...
	if (match_arg(params[0], 'g', ("get"), 2) || match_arg(params[0], 'd', ("display"), 2)) {	/* get/display */
		...
		jabber_write(session, "<iq type='get' id='%s'><query xmlns='jabber:iq:private'><%s/></query></iq>", id, namespace);
		return 0;
	}
...
/* Synchronize config (?) */
		if (config) {
			...
			for (pl = plugins; pl; pl = pl->next) {
				plugin_t *p = pl->data;
				GSList *vl;
				jabber_write(session, "<plugin xmlns='ekg2:plugin' name='%s' prio='%d'>", p->name, p->prio);
back:
				for (vl = variables; vl; vl = vl->next) {
					...
					switch (v->type) {
						...
						case(VAR_DIR):
						case(VAR_THEME):
							if (*(char **) v->ptr)	jabber_write(session, "<%s>%s</%s>", tname, *(char **) v->ptr, tname);
							else			jabber_write(session, "<%s/>", tname);
							break;
						case(VAR_INT):
						case(VAR_BOOL):
							jabber_write(session, "<%s>%d</%s>", tname, *(int *) v->ptr, tname);
							break;
			...
			for (s = sessions; s; s = s->next) {
				...
				jabber_write(session, "<session xmlns='ekg2:session' uid='%s' password='%s'>", s->uid, s->password);

				/* XXX, alias, descr, status */
				for (i = 0; (pl->params[i].key /* && p->params[i].id != -1 */); i++) {
					if (s->values[i]) {
						char *esc = jabber_escape(s->values[i]);
						jabber_write(session, "<%s>%s</%s>", pl->params[i].key, esc, pl->params[i].key);
						xfree(esc);
					} else
						jabber_write(session, "<%s/>", pl->params[i].key);
				}

				jabber_write(session, "</session>");
			}
			...
			for (l = j->bookmarks; l; l = l->next) {
				...
				switch (book->type) {
					case (JABBER_BOOKMARK_URL):
					{
						...
						jabber_write(session, "<url name='%s' url='%s'/>", esc_name, book->priv_data.url->url);
						xfree(esc_name);
						break;
					}
					case (JABBER_BOOKMARK_CONFERENCE):
					{
						...
						jabber_write(session, "<conference name='%s' autojoin='%s' jid='%s'>", esc_name,
							book->priv_data.conf->autojoin ? "true" : "false", book->priv_data.conf->jid);
						if (book->priv_data.conf->nick) jabber_write(session, "<nick>%s</nick>", esc_nick);
						if (book->priv_data.conf->pass) jabber_write(session, "<password>%s</password>", book->priv_data.conf->pass);
						jabber_write(session, "</conference>");
						xfree(esc_nick);
						xfree(esc_name);
						break;
					}
...
/* Do what user want */
		if (params[0] && params[1] && params[2]) /* XXX check */
			jabber_write(session, "%s", params[2]);
...
/*			if (end) *end = '\0'; */	/* SEGV? where? why? :( */
			...
			jabber_write(session, "</%s></query></iq>", beg);
	...
	if (match_arg(params[0], 'c', ("clear"), 2)) {						/* clear */
		...
		jabber_write(session, "<iq type='set' id='%s'><query xmlns='jabber:iq:private'><%s/></query></iq>", id, namespace);
		return 0;
	}
...
static COMMAND(jabber_command_register)
{
	...
	if (splitted) {
		...
		for (; (splitted[i] && splitted[i+1]); i+=2) {
			if (use_x_data)
				jabber_write(session, "<field var='%s'><value>%s</value></field>", splitted[i], splitted[i+1]);
			else	jabber_write(session, "<%s>%s</%s>", splitted[i], splitted[i+1], splitted[i]);
		}
...
/* XXX, porobic potwierdzenia ustawiania/ usuwania. oraz jesli nie ma statusu to wyswylic jakies 'no vacation status'... */
	...
	if (!params[0]) {
		jabber_write(session, "<iq type='get' id='%s'><query xmlns='http://jabber.org/protocol/vacation'/></iq>", id);
	} else if (xstrlen(params[0]) == 1 && params[0][0] == '-') {
		jabber_write(session, "<iq type='set' id='%s'><query xmlns='http://jabber.org/protocol/vacation'/></iq>", id);
	} else {
		jabber_write(session,
...
static COMMAND(jabber_muc_command_join) {
	...
	jabber_write(session, "<presence to='%s/%s'><x xmlns='http://jabber.org/protocol/muc'>%s</x></presence>",
...
static COMMAND(jabber_muc_command_part) {
	...
	jabber_write(session, "<presence to='%s/%s' type='unavailable'>%s</presence>", c->name+5, c->priv_data, status ? status : "");
...
static COMMAND(jabber_muc_command_nick) {
	...
	jabber_write(session, "<presence to='%s/%s'/>", conference, tmp);
	xfree(tmp);
...
/*				"<field var='FORM_TYPE'><value>http://jabber.org/protocol/muc#roomconfig/value></field>" */
		...
		for (i=0; (splitted[i] && splitted[i+1]); i+=2) {
			...
			jabber_write(session, "<field var='%s'><value>%s</value></field>", name, value);
...
static COMMAND(jabber_muc_command_affiliation) {	/* %0 [target] %1 [jid] %2 [reason] */
	...
	if (!params[1]) {
		...
		jabber_write(session, "<iq id='%s' to='%s' type='get'><query xmlns='http://jabber.org/protocol/muc#admin'><item affiliation='outcast'/></query></iq>",
			id, c->name+5);
...
/* XXX da, /topic is possible in normal talk too... current limit only to muc. */
	...
	if (!subject) {
		...
		jabber_write(session, "<message to='%s' type='groupchat'><subject>%s</subject></message>", c->name+5, subject);
		xfree(subject);
	}
...
static COMMAND(tlen_command_alert) {
	...
	jabber_write(session, "<m to='%s' tp='a'/>", target+5);	/* sound alert */
